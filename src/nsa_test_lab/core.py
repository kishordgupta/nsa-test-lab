# -*- coding: utf-8 -*-
"""Negative Selection Algorithm (NSA) family.

This module provides standalone implementations of the major NSA families
described in the NSA review literature: binary/string NSA, real-valued NSA,
V-detector, grid/matrix NSA, adaptive/dynamic NSA, optimization-based NSA,
clustering-based NSA, density/antigen NSA, ensemble NSA, boundary-aware NSA,
and online/feedback variants.

The classes intentionally follow the common anomaly-detector conventions:
...
* ``predict(X)`` returns ``1`` for anomalies and ``0`` for inliers.
...
paper. They expose the same operational idea in a single scikit-learn-like API.
"""
...
class _StandaloneBaseDetector(BaseEstimator):
    """Small anomaly-detector base with PyOD-style labels.

    The upstream implementation lived inside PyOD and used ``BaseDetector`` for
    thresholding. This standalone package keeps the public behavior while
    avoiding a runtime PyOD dependency.
    """
    ...
    def _set_n_classes(self, y: Optional[ArrayLike]) -> None:
        self._classes = 2 if y is None else len(np.unique(y))

    def _process_decision_scores(self) -> None:
        scores = np.asarray(self.decision_scores_, dtype=float)
        if scores.ndim != 1:
            raise ValueError("decision_scores_ must be a one-dimensional array")
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        """Predict binary outlier labels, where ``1`` means anomaly."""
    ...
    def fit_predict(self, X: ArrayLike, y: Optional[ArrayLike] = None) -> np.ndarray:
        """Fit the detector and return training labels."""
        return self.fit(X, y).labels_


class NegativeSelection(_StandaloneBaseDetector):
